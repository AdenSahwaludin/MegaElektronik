<template>
  <div class="fixed inset-0 w-full bg-gradient-to-br from-orange-50 via-amber-50/50 to-orange-50 flex flex-col overflow-hidden pb-[env(safe-area-inset-bottom)]">
    ...
    <div class="flex-1 flex flex-col min-h-0 overflow-hidden">
      ...
      <div
        class="flex-1 overflow-y-auto px-2 sm:px-3 lg:px-5 pt-3 overscroll-contain touch-pan-y"
        :style="{ 
      ...
      >
        ...
        <!-- Header Kolom Produk (Tabel untuk Mobile & Desktop) -->
        <div
          v-show="!showDelayedLoading && filteredProducts.length > 0"
          class="grid grid-cols-[1fr_90px_100px] sm:grid-cols-[36px_minmax(140px,1fr)_110px_140px_170px_200px] gap-2 sm:gap-3 px-3 sm:px-4 py-2 bg-orange-100/70 rounded-xl border border-orange-200/80 text-[10px] sm:text-[11px] font-extrabold text-orange-900 uppercase tracking-wider mb-1 max-w-7xl mx-auto w-full items-center shadow-2xs"
        >
          <div class="hidden sm:block"></div>
          <div>Nama Barang</div>
          <div class="hidden sm:block">Merek</div>
          <div class="hidden sm:block">Model</div>
          <div class="text-center">Harga</div>
          <div class="text-center">Aksi</div>
        </div>

        <!-- Product List View (Table Row Layout on Mobile & Desktop) -->
        <div
          v-show="!showDelayedLoading && filteredProducts.length > 0"
          class="flex flex-col gap-1.5 sm:gap-2 max-w-7xl mx-auto w-full pb-2"
        >
          <div
            v-for="product in filteredProducts"
            :key="product.id"
            :class="[
              'group bg-white rounded-xl border border-gray-200/80 hover:border-orange-300/90 shadow-2xs hover:shadow-md transition-all duration-150 p-2.5 sm:p-3 lg:px-4 text-left relative overflow-hidden',
              'grid grid-cols-[1fr_90px_100px] sm:grid-cols-[36px_minmax(140px,1fr)_110px_140px_170px_200px] gap-2 sm:gap-3 items-center',
              product.stock === 0
          ...
          >
            ...
            <!-- 1. Icon Indicator (Desktop only) -->
            <div
              :class="[
                'hidden sm:flex w-9 h-9 rounded-lg items-center justify-center shrink-0 text-xs font-bold transition-colors',
                product.stock === 0
            ...
            <!-- 2. Nama Barang & Badges (Stok, Merek, Model di Mobile) -->
            <div class="min-w-0 flex flex-col justify-center">
              <h3 class="font-bold text-xs sm:text-sm text-gray-800 group-hover:text-orange-600 transition-colors leading-snug truncate" :title="product.name">
                {{ product.name }}
              </h3>
              <div class="mt-0.5 flex flex-wrap items-center gap-1">
                ...
                <!-- Merek Badge on Mobile -->
                <span
                  v-if="product.brand && product.brand !== 'No Brand' && product.brand.trim() !== ''"
                  class="sm:hidden px-1.5 py-0.2 rounded bg-gray-100 text-gray-600 text-[10px] font-bold border border-gray-200/60 truncate max-w-[80px]"
                >
                  {{ product.brand }}
                </span>

                <!-- Model Badge on Mobile -->
                <span
                  v-if="product.model && product.model.trim() !== '' && product.model !== '-' && product.model.toLowerCase() !== 'standar' && product.model.toLowerCase() !== 'standard'"
                  class="sm:hidden px-1.5 py-0.2 rounded bg-gray-100 text-gray-500 text-[10px] font-semibold border border-gray-200/60 truncate max-w-[80px]"
                >
                  {{ product.model }}
                </span>
              </div>
            ...
            <!-- 3. Merek (Desktop) -->
            <div class="hidden sm:block min-w-0 text-xs font-semibold text-gray-600">
            ...
            <!-- 4. Model (Desktop) -->
            <div class="hidden sm:block min-w-0 text-xs font-semibold text-gray-500">
            ...
            <div class="min-w-0 text-center">
              <p class="text-xs sm:text-base font-black text-orange-600 leading-tight">
                {{ formatCurrency(product.askingPrice) }}
              </p>
              <p class="text-[9px] sm:text-[11px] text-gray-400 font-semibold mt-0.5">
            ...
            <div class="flex items-center justify-center gap-1 shrink-0">
              <button
                ...
                class="px-2 py-1.5 text-[9px] sm:text-[10px] font-bold text-blue-700 bg-blue-50 hover:bg-blue-600 hover:text-white border border-blue-200 hover:border-blue-600 rounded-lg sm:rounded-xl transition-all active:scale-95 flex items-center gap-0.5 shadow-2xs"
                title="Tambah Jasa Service"
              >
                <Icon name="lucide:wrench" class="w-3 h-3 sm:w-3.5 sm:h-3.5" />
                <span class="hidden sm:inline">Svc {{ formatCurrency(product.servicePrice) }}</span>
              </button>
              ...
              <button
                ...
                :class="[
                  'px-2.5 sm:px-3.5 py-1.5 sm:py-2 text-xs font-bold rounded-lg sm:rounded-xl transition-all duration-150 flex items-center gap-1 shadow-2xs active:scale-95',
                  product.stock === 0
                    ? 'bg-gray-100 text-gray-400 cursor-not-allowed border border-gray-200'
                    : 'bg-orange-500 hover:bg-orange-600 text-white shadow-orange-500/20 group-hover:shadow-md'
                ]"
              >
                <Icon name="lucide:plus" class="w-3.5 h-3.5 sm:w-4 sm:h-4" />
                <span>Tambah</span>
              </button>
            </div>
          </div>
            ...
            <div class="flex items-center gap-3 mb-2.5">
              ...
              <div class="flex items-center gap-1 flex-1 min-w-0">
                <button
                  ...
                  class="h-9 px-2 min-w-[38px] bg-orange-100 hover:bg-orange-200 active:bg-orange-300 text-orange-800 font-bold rounded-lg transition active:scale-95 flex items-center justify-center text-xs shrink-0 select-none touch-manipulation border border-orange-200"
                ...
                <div class="relative flex-1 min-w-0">
                  ...
                  <input
                    ...
                    type="text"
                    inputmode="numeric"
                    class="w-full pl-7 pr-2 py-1.5 border border-orange-200 rounded-lg font-bold text-sm text-right focus:outline-none focus:ring-2 focus:ring-orange-400 bg-orange-50/50 transition h-9"
                  />
                </div>
                <button
                  type="button"
                  @click="adjustPrice(item.id, item.soldPrice, 10000)"
                  class="h-9 px-2 min-w-[38px] bg-orange-500 hover:bg-orange-600 active:bg-orange-700 text-white font-bold rounded-lg transition active:scale-95 flex items-center justify-center text-xs shrink-0 select-none touch-manipulation shadow-xs"
